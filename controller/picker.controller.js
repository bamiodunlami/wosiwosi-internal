...
const renderOrderPage =  async (req, res)=>{
    if(req.isAuthenticated()){
        fs.readFile(appRoot+"/public/data/orderToProcess.json" , async (err, file)=>{
        if(err) console.log(err)
        let setting = JSON.parse(file)

        let fromDate=setting.fromDate 
        let toDate=setting.toDate
        let fromTime=setting.fromTime
        let toTime=setting.toTime

        // console.log(req.query)
        // ${date.toLocaleTimeString()}
        let pageNumber = setting.page
